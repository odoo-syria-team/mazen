...
class ProductTemplate(portal.CustomerPortal):
    ...
    def get_products(self, sortby='name', page=1, search='', search_in='Product'):
        ...
        try:
            if request.env.user.partner_id.partner_type not in ['distributor', 'super_distributor']:
                raise AccessDenied()
            ...
            return request.render('bbook_distributor_market_management.product_list', {
    ...
    def get_product_record(self, product_id, **post):
        ...
        if request.httprequest.method == 'POST':
            try:
                ...
                if product_id == -1:
                    # ADD CHECK ON DESTERBURTER TYPE
                    user_product_id = request.env['product.template'].sudo().create(
                        {
                            'name': data['name'],
                            'partner_id': request.env.user.partner_id.id,
                            'company_id': request.env.user.company_id.id,
                            'product_state': 'approved' if request.env.user.partner_id.partner_type == 'super_distributor' else 'pending'
                        })

                    if request.env.user.partner_id.partner_type == 'distributor':
            ...
            try:
                ...
                return request.render('bbook_distributor_market_management.product_form', {
                    'user_id': request.env.user,
